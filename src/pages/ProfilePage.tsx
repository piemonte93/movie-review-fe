...
import { UserProfile, UserActivity, User } from "../types/user";
...
const ProfilePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchUserData = async () => {
      // 현재 경로가 /profile이 아니면 데이터를 로드하지 않음
  ...
  // username이 제대로 표시되도록 useEffect 추가
  useEffect(() => {
    if (profileData?.user && profileData.user.username) {
      // 프로필 데이터가 로드되었을 때 초기 사용자명 설정
      setEditUsername(profileData.user.username);
    } else if (user?.username) {
      // 프로필 데이터가 없을 경우 현재 로그인한 사용자 정보 사용
                ...
                {like.type === "movie" ? (
                  <div className="flex">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400 mt-2">{like.date}</p>
                    </div>
                  </div>
                ) : (
                  <div>
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      // 프로필 데이터도 업데이트 (UserProfile 타입에 맞게)
          ...
          <div className="flex-grow">
            <div className="flex items-center justify-between mb-4">
              <h1 className="text-xl md:text-2xl font-medium">
                {profileData?.user?.username || user?.username || "사용자"}
              </h1>
            ...
            <div className="overflow-y-auto max-h-80">
              {followingList.length > 0 ? (
                followingList.map((user) => {
                  ...
                  const isFollowingBack = followersList.some(
                      ...
                      <FollowButton
                        ...
                        onToggleFollow={(userId, isFollowing) => {
                          if (!isFollowing) {
                            ...
                            // 팔로잉 목록에서 제거
                            setFollowingList(
                              followingList.filter((item) => item.id !== userId)
                            );
                            // 팔로잉 수 감소
                            if (profileData) {
                              setProfileData({
                      ...
                      <FollowButton
                        ...
                        onToggleFollow={(userId, isFollowing) => {
                          ...
                          if (isFollowing) {
                            // 팔로잉 목록에 추가
                            if (
                              !followingList.some((item) => item.id === userId)
                            ) {
          ...
          <div className="bg-white rounded-lg w-full max-w-md overflow-hidden">
            ...
            <div className="p-6">
              <div className="mb-6 flex flex-col items-center">
                <div
                  className="relative h-24 w-24 rounded-full overflow-hidden bg-gray-200 cursor-pointer group border border-gray-300 mb-2"
                  onClick={handleProfileImageClick}
                >
                  {user?.profileImageUrl ? (
                    <img
                ...
                <span className="text-sm text-blue-500">프로필 사진 변경</span>
              </div>
              ...
              <div className="mt-6 flex justify-end space-x-3">
                ...
                <button
                  className="px-4 py-2 text-sm bg-blue-500 text-white rounded-md hover:bg-blue-600 flex items-center justify-center"
...
const UserHoverCard: React.FC<UserHoverCardProps> = ({ userId, username }) => {
  ...
  const loadUserData = () => {
    ...
    setIsFollowingBack(
      isCurrentlyFollowing && mockFollowers.includes(userIdNum)
    );
    ...
    try {
      ...
      if (userData) {
        if (result.isFollowing) {
          ...
          setUserData({
            ...userData,
            followerCount: Math.max(0, (userData.followerCount || 0) - 1),
          });
        }
      }
    } catch (error) {
  ...
  return (
    ...
    >
      <span className="font-medium mr-2 cursor-pointer text-blue-600 hover:underline">
        {username}
      </span>
