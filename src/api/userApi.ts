...
export const getUserProfile = async (): Promise<UserProfile> => {
  try {
    ...
    let localUsername = "사용자";
    let localEmail = "user@example.com";
    let localProfileImageUrl = undefined;
    let localBio = "";
    
    if (userStr) {
      try {
        const localUser = JSON.parse(userStr);
        localUsername = localUser.username || "사용자";
        localEmail = localUser.email || "user@example.com";
        localProfileImageUrl = localUser.profileImageUrl;
        localBio = localUser.bio || "";
      } catch (e) {
        console.error("로컬 스토리지 사용자 정보 파싱 오류:", e);
      }
    }
    
    // 백엔드 연결이 되지 않으므로 목업 데이터 + 로컬 스토리지 데이터 반환
    return {
      user: {
        id: 1,
